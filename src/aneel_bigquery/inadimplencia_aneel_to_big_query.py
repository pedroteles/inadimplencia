...
TABLE_ID = "aneel_raw_data"
LOCAL_DATA_PATH = r"C:\Users\pedro\OneDrive - Sunflow\Documentos\SmartSun\Portifólio\inadimplencia\bases\aneel\inadimplencia.csv"
...
def process_csv():
    ...
    try:
        ...
        df = pd.read_csv(
            LOCAL_DATA_PATH,
            sep=';',  # Ajuste o separador se necessário (vírgula, ponto-e-vírgula, etc.)
            decimal=',',
            encoding='ISO-8859-1',
            dtype={
                'SigAgente': str,
                ...
                'SigIndicador': str,
                'AnoIndice': int,
                'NumPeriodoIndice': int,
                'VlrIndiceEnviado': float
            },
        ...
        parquet_filename = "aneel_raw_data.parquet"
...
def load_to_bigquery(gcs_uri):
    ...
    try:
        ...
        job_config = bigquery.LoadJobConfig(
            ...
            clustering_fields=["anoindice", "numperiodoindice", "sigagente"]
        )
